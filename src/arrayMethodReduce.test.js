...
test(`reduce2 doesn't call default reduce`, () => {
  expect([].reduce2.toString().includes('.reduce(')).toBe(false);
});

test('for .reduce2((sum, x) => sum + x, 0), numbers', () => {
  expect(sourceNumbers.reduce2((sum, x) => sum + x, 0)).toBe(60);
});

test('for .reduce2((sum, x) => sum + x, ""), strings', () => {
  expect(sourceStr.reduce2((sum, x) => sum + x, '')).toBe('abcd');
});

test('takes first item as a `startValue` if not given, numbers', () => {
  expect(sourceNumbers.reduce2((sum, x) => sum + x)).toBe(60);
});
...
test('returns `startValue` for []', () => {
  expect([].reduce2((x) => x, 999)).toBe(999);
});

test('for (sum, item, index) => sum + index', () => {
  expect(sourceNumbers.reduce2((sum, x, i) => sum + i)).toBe(6);
});

test('should handle undefined if it was passed as initialValue', () => {
  expect(sourceStr.reduce2((acc, current) => acc + current, 'undefined')).toBe(
    'undefinedabcd'
  );
});

// eslint-disable-next-line max-len
test('for .reduce2((sum, x, i, arr) => sum + (arr === sourceStr ? 1 : 0), 0)', () => {
  expect(
    sourceStr.reduce2((sum, x, i, arr) => sum + (arr === sourceStr ? 1 : 0), 0)
  ).toBe(4);
});
